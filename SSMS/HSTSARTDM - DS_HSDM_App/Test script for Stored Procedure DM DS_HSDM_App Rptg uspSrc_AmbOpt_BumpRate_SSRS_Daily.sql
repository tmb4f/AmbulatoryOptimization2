...
--ALTER PROCEDURE [Rptg].[uspSrc_AmbOpt_BumpRate_SSRS_Daily]
--    (
...
--AS 
--/**********************************************************************************************************************
--WHAT: Create procedure Rptg.uspSrc_AmbOpt_BumpRate_SSRS_Daily
--WHO : Tom Burgan
--WHEN: 6/3/19
...
--                
--      OUTPUTS:  [Rptg].[uspSrc_AmbOpt_BumpRate_SSRS_Daily]
--
--------------------------------------------------------------------------------------------------------------------------
--MODS: 	
--         06/04/2019 - TMB - create stored procedure
--************************************************************************************************************************
...
SELECT 
	   ...
	   CASE WHEN evnts.appt_event_Provider_Canceled = 1 AND evnts.Cancel_Lead_Days <= 45 THEN 1 ELSE 0 END AS [Bump],
...
WHERE ((evnts.appt_event_Canceled = 0)
       OR ((evnts.appt_event_Canceled_Late = 1)
		   OR (evnts.appt_event_Provider_Canceled = 1) -- Show all provider-initated cancellations, regardless of lead days
		  )
	  )
      ...
      (
          @in_depid = 0
          OR (evnts.epic_department_id IN (SELECT epic_department_id FROM @tab_depid))
      )
      AND
      (
          @in_pods = 0
      ...
      (
          @in_podid = 0
          OR (evnts.pod_id IN (SELECT pod_id FROM @tab_podid))
      )
      AND
      (
          @in_hubs = 0
      ...
      (
          @in_hubid = 0
          OR (evnts.hub_id IN (SELECT hub_id FROM @tab_hubid))
      )
      AND
      (
          @in_servLine = 0
      ...
      (
          @in_somdepid = 0
          OR (evnts.som_department_id IN (SELECT som_department_id FROM @tab_somdepid))
      )
      AND
      (
          @in_somdivs = 0
      ...
      (
          @in_somdivid = 0
          OR (evnts.som_division_id IN (SELECT som_division_id FROM @tab_somdivid))
      )
